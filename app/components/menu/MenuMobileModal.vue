<script setup lang="ts">
  ...
  const computedLinks = computed(() => {
    return links.map((link) => ({
      ...link,
      isActive:
        route.fullPath === link.href || (link.href !== '/' && route.fullPath.startsWith(link.href)),
    }));
  });
</script>
  ...
  >
    <v-card class="menu__body">
      <div class="menu__header mb-4">
        ...
        <v-btn class="modal__close-btn mt-2 mb-2" icon @click="closeMobileMenuModal" variant="text">
          <v-icon :icon="mdiClose" size="30px" />
        </v-btn>
      </div>
      <div>
        <div class="menu__auth auth mb-2">
          <v-card
            class="auth__item"
            v-if="!authUser"
            v-ripple
