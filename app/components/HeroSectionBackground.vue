...
<template>
  <header
    ...
    :style="{
      ...
      '--adaptive-overlay-from':
        props.adaptive?.overlay?.from || props.overlay?.from || 'rgba(0, 0, 0, 0.6)',
      '--adaptive-overlay-to':
        props.adaptive?.overlay?.to || props.overlay?.to || 'rgba(0, 0, 0, 0.2)',
  ...
  @media (max-width: 600px) {
    ...
    .hero__body {
      background: linear-gradient(
        to bottom,
        var(--adaptive-overlay-from),
        var(--adaptive-overlay-to)
      );
    }
  }
</style>
