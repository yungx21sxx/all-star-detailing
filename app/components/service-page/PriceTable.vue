    ...
    <div class="price__wrapper">
      <div class="price__scroller" ref="price_table_node">
        <v-table theme="dark" class="service__table table" v-if="priceTable">
          <thead>
            <tr>
              <th class="table__title">Услуга / Тип машины</th>
          ...
          <tbody>
            <tr v-for="{ name, prices } of priceTable.services">
              <td class="table__service">{{ name }}</td>
              <td v-for="price of prices" v-if="prices.length > 1">{{ price }} ₽</td>
              <td v-else colspan="4" style="text-align: center; vertical-align: middle">
                {{ prices[0] }} ₽
              </td>
            </tr>
          </tbody>
