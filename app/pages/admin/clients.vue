<script setup lang="ts">
  ...
  const { data: users } = await useAsyncData('users', () => $fetch('/api/user/get-users'));
  ...
  watch(phoneSearchStr, () => {
    if (phoneSearchStr.value !== '') {
      searchedUsers.value = users.value.filter((user) =>
        user.phoneRow.includes(phoneSearchStr.value),
      );
    } else {
...
<template>
  <div class="wrapper admin">
    <v-expansion-panels class="mb-4">
      <v-expansion-panel title="Создать польователя">
        <v-expansion-panel-text>
          <form @submit.prevent="createUser">
            <phone-input
              ...
              @update-row-value="setCreateFormPhoneRow"
            />
            <v-text-field
              label="Начислить бонусов"
              variant="outlined"
              density="compact"
              type="number"
              v-model="userCreateForm.balance"
            />
    ...
    <PhoneInput v-model="phoneStr" @update-row-value="setSearchPhoneStr" />
    <v-expansion-panels>
      <v-expansion-panel v-for="user of searchedUsers" :key="user.id" :title="user.phone">
        <v-expansion-panel-text>
          <div class="admin__user-info">
            ...
            <v-text-field
              ...
              density="compact"
              type="number"
              v-model="currentBalanceInput"
            />
            <div class="admin__control">
              <v-btn color="#f1aa34" @click="changeBalance('ADD', user.id, user.balance)"
                >Пополнить</v-btn
              >
              <v-btn color="#f1aa34" @click="changeBalance('TAKE', user.id, user.balance)"
                >Снять</v-btn
              >
            </div>
          </div>
