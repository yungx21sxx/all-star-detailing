<template>
  <div class="wrapper clients">
    <header class="clients__header">
      ...
      <v-btn color="#f1aa34" class="clients__btn" href="/admin/portfolio/new"
        >Добавить машину</v-btn
      >
    </header>
    <div class="clients__grid client">
      <v-card v-for="{ id, photos, name, description } in clients" :key="id">
        <v-img
  ...
  <ModalLayout v-model="modalEditOpened" width="auto">
    <template #title>
      <h3 class="ml-7">Редактировать</h3>
    </template>
    <v-card>
      <v-card-item>
        <v-text-field
          v-model="currentPorfolioItem.name"
          variant="outlined"
          title="Заголовок"
        ></v-text-field>
        <text-editor v-model="currentPorfolioItem.description" />
      </v-card-item>
      <v-card-actions> </v-card-actions>
    </v-card>
    <v-btn @click="savePortfolioItem" color="#f1aa34" class="modal__btn">Сохранить</v-btn>
  </ModalLayout>
</template>
