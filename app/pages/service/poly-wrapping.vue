<script setup lang="ts">
  ...
  useSeoMeta({
    title:
      'Защитная пленка на авто в СПб | Антигравийное бронирование кузова «под ключ» | All-Star Detailing',
    description:
      '🛡️Самые доступные цены на защитную пленку в Санкт-Петербурге! Защита кузова от сколов и царапин без переплат. Гарантия 3 года, только сертифицированные пленки. Бесплатный расчет + скидка 5% новым клиентам!',
    ogTitle:
      'Защитная пленка на авто в СПб | Антигравийное бронирование кузова «под ключ» | All-Star Detailing',
    ogDescription:
      '🛡️Самые доступные цены на защитную пленку в Санкт-Петербурге! Защита кузова от сколов и царапин без переплат. Гарантия 3 года, только сертифицированные пленки. Бесплатный расчет + скидка 5% новым клиентам!',
  ...
  const priceTable: IPriceTable | undefined = PRICE_TABLE.find(
    (table) => table.serviceID === 'poly-wrapping',
  );
  ...
  const firstHalf = computed(() =>
    carPartsArray.value.slice(0, Math.ceil(carPartsArray.value.length / 2)),
  );
  const secondHalf = computed(() =>
    carPartsArray.value.slice(Math.ceil(carPartsArray.value.length / 2)),
  );
...
<template>
  ...
  >
    ...
    <p class="banner__text">
      Полиуретановые пленки высочайшего качества, надежная защита кузова автомабиля с гарантией
      <br />
      <strong>3 года!</strong>
    </p>
    <v-btn variant="flat" color="#f1aa34" @click="openSubmitModal('Оклейка защитной пленкой')"
      >Записаться на услугу
    </v-btn>
  </HeroSectionBackground>
  <main class="service">
    <div class="wrapper price block-end-margin block" id="price">
      <VintersectAnimation>
        <h1 class="title">Цены на оклейку пленкой</h1>
        <div class="under-line mb-6"></div>
        <p class="subtitle mb-4">
          Оклейка защитной <strong>прозрачной</strong> или <strong>матовой</strong> пленкой
        ...
        <div class="price__grid">
          <div class="card card1" itemscope itemtype="https://schema.org/Product">
            ...
            <div class="card__time">Выполним за <span itemprop="productionTime">2 дня</span></div>
            <v-divider />
            <div class="card__description" itemprop="description">
              Передняя часть/зоны риска: Бампер, капот, крылья, фары, полоса над лобовым, стойки
              лобового стекла, зеркала.
            </div>
          </div>
          ...
          <div class="card card2" itemscope itemtype="https://schema.org/Product">
            ...
            <div class="card__time">Выполним за <span itemprop="productionTime">8 дней</span></div>
            <v-divider />
            <div class="card__description" itemprop="description">
              <ul class="card__list">
                <li class="list-item">
          ...
          <div class="card card3" itemscope itemtype="https://schema.org/Product">
            ...
            <div class="card__time">Выполним за <span itemprop="productionTime">1 час</span></div>
          </div>
        </div>
        <p class="subtitle mb-6 mt-8">
          Начислим <strong>1000 бонусов</strong> при регистрации и вернем <br />
          <strong>5%</strong> от суммы оказанных услуг
        </p>
        <v-btn
      ...
      <VintersectAnimation>
        <div class="tables-grid">
          <v-table class="poly-table">
            <tbody>
            <tr
                v-for="([name, price], index) in firstHalf"
                :key="'first-' + index"
                itemscope
            ...
            >
              ...
              <td>
                <span itemprop="offers" itemscope itemtype="https://schema.org/Offer">
                  от <span itemprop="price">{{ Intl.NumberFormat('ru-RU', {
                  useGrouping: true,
          ...
          <v-table class="poly-table">
            <tbody>
            <tr
                v-for="([name, price], index) in secondHalf"
                :key="'second-' + index"
                itemscope
                itemtype="https://schema.org/Product"
            >
        ...
        <p class="text mb-6">
          // Цена зависит класса автомобиля и производителя пленки. Запишитесь на услугу для получения
          бесплатной консультации
        </p>
        <v-btn
            @click="openSubmitModal('Оклейка защитной пленкой')"
            class="mb-8"
            variant="outlined"
            color="#f1aa34"
            :append-icon="mdiArrowRight"
    ...
    <VintersectAnimation class="bonus block-end-margin block">
      <div class="wrapper">
        ...
        <div class="under-line mb-6"></div>
        <v-timeline class="timeline_desktop" side="end" align="start" direction="horizontal">
          <v-timeline-item
            class="timeline-item"
            size="large"
            fill-dot
          ...
          >
            ...
            <p>
              Тщательная мойка, обезжиривание и полировка поверхности. Демонтаж элементов, мешающих
              оклейке, для идеального прилегания пленки.
            </p>
          </v-timeline-item>
          <v-timeline-item fill-dot size="large" dot-color="#f1aa34" :icon="mdiContentCut">
            <h3 class="bonus__subtitle_orange">Нанесение пленки</h3>
            <p>
              Аккуратная оклейка каждой детали с использованием профессионального оборудования.
              Удаление пузырей и складок для идеального результата.
            </p>
          </v-timeline-item>
          <v-timeline-item fill-dot size="large" dot-color="#f1aa34" :icon="mdiCheck">
            <h3 class="bonus__subtitle_orange">Финальная обработка</h3>
            <p>
              Фиксация пленки с помощью тепловой обработки. Проверка качества, сборка
              демонтированных элементов и выдача автомобиля клиенту.
            </p>
          </v-timeline-item>
        </v-timeline>
        <v-timeline class="timeline_mobile" side="end" align="start" direction="vertical">
          <v-timeline-item
            class="timeline-item"
            size="large"
            fill-dot
    ...
    <VintersectAnimation class="wrapper about block-end-margin block">
      <div class="desc">
        ...
        <div class="under-line"></div>
        <p class="mb-6">
          Мы наносим полиуретановые и виниловые пленки высочайшего качества, что делает их не только
          надежным способом защиты, но и украшением для кузова любого автомобиля.
        </p>
        ...
        <v-lazy :min-height="157">
          <iframe
            class="review__yandex"
            style="width: 100%; border: 0 solid #e6e6e6; border-radius: 8px; box-sizing: border-box"
            src="https://yandex.ru/maps-reviews-widget/181191680424?comments"
          ></iframe>
        </v-lazy>
      </div>
      <div v-intersect.once="handleIntersection" class="video-container">
        ...
        <div v-if="!videoLoaded" class="video-placeholder">
          <v-progress-circular color="#f1aa34" indeterminate size="50" />
        </div>
      </div>
    </VintersectAnimation>
      ...
      <div class="advantages__grid">
        ...
        >
          <div class="advantage__bottom">
            <img src="/poly-wrapping/icons/work.svg" class="advantage__icon" alt="Опыт мастеров — 7+ лет, более 100 довольных клиентов" />
        ...
        >
          <div class="advantage__bottom">
            <img src="/poly-wrapping/icons/garanty.svg" class="advantage__icon" alt="Гарантия на материалы и работу — 3 года" />
            <p class="advantage__text">
              Гарантия на материалы и <br />
              работу — 3 года
            </p>
          </div>
        </VImg>
        ...
        >
          <div class="advantage__bottom">
            <img src="/poly-wrapping/icons/wrap.svg" class="advantage__icon" alt="Только сертифицированные пленки" />
        ...
        >
          <div class="advantage__bottom">
            <img src="/poly-wrapping/icons/check.svg" class="advantage__icon" alt="Бесплатная диагностика и уход после оклейки" />
    ...
    <VintersectAnimation class="wrapper vinyl block">
      <div class="vinil-wrapping margin-block-end">
        <v-img
          ...
          class="service__img"
          src="/vinyl-wrapping/slider/4.webp"
          cover
          alt="ОКЛЕЙКА АВТО ПОЛИУРЕТАНОВОЙ ПЛЕНКОЙ"
        />
        <section>
          <h2 class="title service__title_white">Оклейка цветной пленкой</h2>
          <div class="under-line mb-6"></div>
          <div class="benefits">
            <li
              class="benefit-item"
          ...
          <div class="benefit-item consultation">
            ...
            <div>
              ...
              <v-btn
                ...
                variant="outlined"
                color="#f1aa34"
                @click="openSubmitModal('Полировка кузова')"
                >Записаться на услугу</v-btn
              >
            </div>
          </div>
        </section>
